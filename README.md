...
dvc pull
# ВНИМАНИЕ! Возникнет ошибка "ERROR: failed to pull data from the cloud" - игнорируйте её
# ошибка возникает т.к. файлы не загружены в удалённое хранилище (remote = localstorage)
# в данном примере данных хранятся в папке "dvc-storage"
...
Эта команда запускает локальный веб-сервер MLflow, который использует SQLite базу данных (`mlflow.db`) для хранения информации об экспериментах и запусках. После запуска команды откройте браузер и перейдите на адрес http://localhost:5000
